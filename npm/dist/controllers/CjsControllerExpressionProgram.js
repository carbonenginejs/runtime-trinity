...
const CONTROLLER_EXPRESSION_TERMS = [["Controller", "StateTime", "time in seconds the current state is running"], ["Controller", "AnimationTime", "geometry animation duration for the given name"], ["Controller", "CurveSetTime", "duration in seconds of the named curve set"], ["Controller", "GetExternalControllerVariable", "external controller variable lookup"], ["Controller", "IsAnimationPlaying", "returns 1 if the named animation layer is playing"], ["Controller", "ShipSpeed", "owning ship speed"], ["Controller", "ShipMaxSpeed", "owning ship maximum speed"], ["Controller", "ShipBoosterIntensity", "owning ship booster intensity"], ["Controller", "Random", "random integer from min to max - 1"], ["Controller", "KillCount", "owning ship kill count"], ["Controller", "BoundingSphereRadius", "owning object's bounding sphere radius"], ["GraphicSettings", "ShaderQuality", "user shader quality setting"], ["DateTime", "IsWeekend", "returns 1 on Saturday or Sunday"], ["DateTime", "ServerYear", "current server year"], ["DateTime", "ServerMonth", "current server month"], ["DateTime", "ServerDay", "current server day of month"], ["DateTime", "ServerDayOfWeek", "current server day of week"], ["DateTime", "ServerHour", "current server hour"], ["DateTime", "ServerMinute", "current server minute"], ["DateTime", "ServerSecond", "current server second"], ["DateTime", "ServerTimePhase", "seconds phase in a server-time period"], ["DateTime", "ServerTimeGreaterThan", "server-time comparison"], ["DateTime", "ServerTimeLessThanOrEqual", "server-time comparison"], ["DateTime", "ServerTimeEqual", "server-time equality comparison"], ["DateTime", "DaysSinceServerTime", "days since the supplied server date"]].map(([group, name, description]) => ({
  group,
  name,
  description,
  kind: "function"
}));
const CURVE_EXPRESSION_TERMS = [["Random", "fractal", "function", "x, alpha, beta, n", "fractal noise"], ["Random", "noise", "function", "x", "simple one-octave noise"], ["Random", "randomConstant", "function", "a, b", "random per-curve constant in range [a, b)"], ["Random", "randconst", "function", "a, b", "random per-curve constant in range [a, b)"], ["Random", "random", "function", "a, b", "random value in range [a, b)"], ["Random", "randhash", "function", "a, b, x", "random value in range [a, b) based on value x"], ["Inputs", "input", "function", "n", "n-th input curve value at current time"], ["Inputs", "inputAt", "function", "n, t", "input curve value at time t"], ["Math", "clamp", "function", "x, min, max", "value x clamped to [min, max] range"], ["Inputs", "input1", "variable", null, "input1 attribute"], ["Inputs", "input2", "variable", null, "input2 attribute"], ["Inputs", "input3", "variable", null, "input3 attribute"], ["Inputs", "input4", "variable", null, "input4 attribute"], ["Inputs", "time", "variable", null, "current time"], ["Math", "pi", "variable", null, "Pi value"], ["Math", "pi2", "variable", null, "Pi x 2 value"]].map(([group, name, kind, parameters, description]) => ({
  group,
  name,
  kind,
  ...(parameters ? {
...
class CjsControllerExpressionProgram {
  ...
  static compileCached(state, expression, emptyValue = 0, functions) {
    if (!state.program || state.source !== expression) {
      state.program = this.Compile(expression, {
        emptyValue,
        functions
      });
  ...
  static makeActionContext(controller, owner, state, extra = {}) {
    ...
    if (controller?.GetExpressionContext) {
      return controller.GetExpressionContext(owner, null, {
        ...extra,
        stateTime
      });
    }
  ...
  static getCurveTermInfo(options = {}) {
    const terms = CURVE_EXPRESSION_TERMS.map(term => ({
      ...term
    }));
...
class CjsControllerExpressionCompileError extends Error {
  expression;
  reason;
  position;
  constructor(data) {
...
class CjsControllerExpressionEvaluateError extends Error {
  expression;
  reason;
  constructor(data) {
...
export { CjsControllerExpressionCompileError, CjsControllerExpressionEvaluateError, CjsControllerExpressionProgram };
//# sourceMappingURL=CjsControllerExpressionProgram.js.map
