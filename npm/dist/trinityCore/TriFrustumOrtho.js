...
class TriFrustumOrtho {
  ...
  SphereTestIgnoreFarPlane(sphere) {
    ...
    const d = outsideMinX * outsideMinX + outsideMinY * outsideMinY + outsideMinZ * outsideMinZ + outsideMaxX * outsideMaxX + outsideMaxY * outsideMaxY;
    if (d > radius * radius) {
      return TriFrustumTestResult.Outside;
    }
    const inside = fromMinX >= radius && fromMinY >= radius && fromMinZ >= radius && fromMaxX >= radius && fromMaxY >= radius;
...
export { TriFrustumOrtho };
//# sourceMappingURL=TriFrustumOrtho.js.map
