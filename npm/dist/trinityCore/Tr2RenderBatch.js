...
class TriRenderBatchAreaBlocksWithSharedMaterial extends TriRenderBatchAreaBlock {
  constructor() {
    super();
...
class Tr2RenderBatch {
  ...
  SetGeometryFromAllocations(vertexDeclaration, vertexAllocation, indexAllocation) {
    this.SetGeometry(vertexDeclaration, vertexAllocation?.GetBuffer?.() ?? vertexAllocation ?? null, vertexAllocation?.GetStride?.() ?? 0, indexAllocation?.GetBuffer?.() ?? indexAllocation ?? null, indexAllocation?.GetStride?.() ?? 0);
  }
...
function CanBeBinned(batch1, batch2) {
  return batch1.shader === batch2.shader && batch1.vertexDeclaration === batch2.vertexDeclaration && batch1.indexStride === batch2.indexStride && batch1.vertexStreams[0] === batch2.vertexStreams[0] && batch1.vertexStreams[1] === batch2.vertexStreams[1] && batch1.renderingMode === batch2.renderingMode;
}
...
function CompareBatches(batch1, batch2) {
...
function Tr2GdprBatchFullPartition(batches) {
  PartitionBatchGroups(batches);
}
function Tr2GdprBatchStableFullPartition(batches) {
  PartitionBatchGroups(batches);
}
...
export { CanBeBinned, CompareBatches, OrderOf, Tr2GdprBatchFullPartition, Tr2GdprBatchStableFullPartition, Tr2RenderBatch, TriRenderBatchAreaBlock, TriRenderBatchAreaBlocksWithSharedMaterial };
//# sourceMappingURL=Tr2RenderBatch.js.map
