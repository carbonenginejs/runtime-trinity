...
export class Tr2StateMachineTransition extends CjsModel
  ...
  #source: ITr2StateMachineState | null = null;
  #destination: ITr2StateMachineState | null = null;
  #program: CjsControllerExpressionProgram | null = null;
  #programSource: string | null = null;
  ...
  @carbon.method
  @impl.adapted
  CanActivate(variableDirtyMask: bigint | number = 0): boolean {
    ...
    if (!DirtyMaskMatches(this.GetVariableMask(), variableDirtyMask)) {
      return false;
    }
  ...
  @carbon.method
  @impl.adapted
  GetSource(): ITr2StateMachineState | null {
    return this.#source;
  }
  ...
  @carbon.method
  @impl.adapted
  GetState(): ITr2StateMachineState | null {
  ...
  GetVariableMask(): bigint {
    ...
    for (const name of this.#variableNames) {
      const index = variableView.find((variable) =>
        variable && typeof variable === "object" && "name" in variable &&
  ...
  EvaluateExpression(expression: string): number {
    ...
    return Number(program.Evaluate(
      this.#getExpressionContext(controller, owner, stateMachine),
    ));
  }
  ...
  #getExpressionContext(
    ...
    return {
      ...
      owner: (owner as Record<string, unknown> | null) ?? undefined,
      stateMachine:
        stateMachine as CjsControllerExpressionContext["stateMachine"],
    };
  }

  #resolveDestination(): ITr2StateMachineState | null {
    const stateMachine = this.#source?.GetStateMachine?.() ?? null;
...
function DirtyMaskMatches(
  ...
  const dirtyMask = typeof dirtyVariables === "bigint"
    ? dirtyVariables
    : BigInt(dirtyVariables);
