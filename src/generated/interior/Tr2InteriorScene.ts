// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Interior/Tr2InteriorScene.h
// Generated by format-carbon carbon-class --emit. Verify against interior/Tr2InteriorScene.json.
import { carbon, impl, io, schema, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";
import { vec3 } from "@carbonenginejs/core-math/vec3";
import { vec4 } from "@carbonenginejs/core-math/vec4";
import type { Vec3, Vec4 } from "@carbonenginejs/core-math/types";

/** Tr2InteriorScene (interior) - generated from schema shapeHash 3a5a36e9.... */
@type.define({ className: "Tr2InteriorScene", family: "interior" })
export class Tr2InteriorScene extends CjsModel
{

  /** m_backgroundCubeMapPath (std::string) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.string
  backgroundCubemapPath = "";

  /** m_visualizeMethod (VisualizeMethod - enum VisualizeMethod) [READWRITE, ENUM, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.int32
  @schema.enum("VisualizeMethod")
  visualizeMethod = 0;

  /** m_curveSets (PTriCurveSetVector) [READ, PERSIST] */
  @io.persist
  @type.list("TriCurveSet")
  curveSets = [];

  /** m_renderShadows (bool) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.boolean
  renderShadows = true;

  /** m_debugRenderShadowMaps (bool) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.boolean
  debugRenderShadowMaps = false;

  /** m_shadowCount (int) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.int32
  shadowCount = 4;

  /** m_minFogDistance (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  minFogDistance = 0;

  /** m_maxFogDistance (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  maxFogDistance = 1000;

  /** m_fogColor (Color) [READWRITE, PERSIST] */
  @io.persist
  @type.color
  fogColor: Vec4 = vec4.fromValues(1, 1, 1, 1);

  /** m_dynamics (PITr2InteriorDynamicVector) [READ, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.list("ITr2InteriorDynamic")
  dynamics = [];

  /** m_lights (PITr2InteriorLightVector) [READ, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.list("ITr2InteriorLight")
  lights = [];

  /** m_maxFogAmount (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  maxFogAmount = 0;

  /** m_debugRenderer (Tr2DebugRendererPtr) [READWRITE] */
  @io.readwrite
  @type.objectRef("Tr2DebugRenderer")
  debugRenderer = null;

  /** m_visibilityResults (Tr2VisibilityResultsPtr) [READ] */
  @io.read
  @type.objectRef("Tr2VisibilityResults")
  visibilityResults = null;

  /** m_ambientColor (Color) [READWRITE, PERSIST] */
  @io.persist
  @type.color
  ambientColor: Vec4 = vec4.create();

  /** m_optimizeShadows (bool) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.boolean
  optimizeShadows = true;

  /** m_shadowSize (int) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.int32
  shadowSize = 1024;

  /** m_lightRenderTargets (PTr2RenderTargetVector) [READ] */
  @io.read
  @type.list("Tr2RenderTarget")
  lightRenderTargets = [];

  /** m_sunDiffuseColor (Color) [READWRITE, PERSIST] */
  @io.persist
  @type.color
  sunDiffuseColor: Vec4 = vec4.createLinear();

  /** m_sunDirection (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  sunDirection: Vec3 = vec3.fromValues(0, 0, 1);

  /** m_sunSpecularColor (Color) [READWRITE, PERSIST] */
  @io.persist
  @type.color
  sunSpecularColor: Vec4 = vec4.fromValues(0.8, 0.8, 0.8, 1);

  /** m_backgroundCubeMapRes (TriTextureResPtr) [READ] */
  @io.read
  @type.objectRef("TriTextureRes")
  backgroundCubemapRes = null;

  /** m_backgroundEffect (Tr2EffectPtr) [READWRITE, PERSIST] */
  @io.persist
  @type.objectRef("Tr2Effect")
  backgroundEffect = null;

  /** Carbon method PickObject -> PickObjectOnly (MAP_METHOD_AND_WRAP_OPTIONAL_ARGS). */
  @carbon.method
  @impl.notImplemented
  PickObject(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "PickObject", args);
  }

  /** Carbon method AddDynamic (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  AddDynamic(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "AddDynamic", args);
  }

  /** Carbon method AddLightSource (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  AddLightSource(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "AddLightSource", args);
  }

  /** Carbon method Pick -> PyPick (MAP_METHOD). */
  @carbon.method
  @impl.notImplemented
  Pick(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "Pick", args);
  }

  /** Carbon method PickObjectAndArea -> PyPickObjectAndArea (MAP_METHOD). */
  @carbon.method
  @impl.notImplemented
  PickObjectAndArea(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "PickObjectAndArea", args);
  }

  /** Carbon method PickPointAndObject -> PyInteriorPickPointAndObject (MAP_METHOD). */
  @carbon.method
  @impl.notImplemented
  PickPointAndObject(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "PickPointAndObject", args);
  }

  /** Carbon method PickObjectUV (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  PickObjectUV(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "PickObjectUV", args);
  }

  /** Carbon method RebuildSceneData (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  RebuildSceneData(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "RebuildSceneData", args);
  }

  /** Carbon method RemoveDynamic (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  RemoveDynamic(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "RemoveDynamic", args);
  }

  /** Carbon method RemoveLightSource (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  RemoveLightSource(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "RemoveLightSource", args);
  }

  /** Carbon method UpdateScene -> UpdateSceneFromScript (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  UpdateScene(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "UpdateScene", args);
  }

  /** Carbon method SetupShadowMaps (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  SetupShadowMaps(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorScene", "SetupShadowMaps", args);
  }

}
