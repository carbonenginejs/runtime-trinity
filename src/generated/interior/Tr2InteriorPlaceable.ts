// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Interior/Tr2InteriorPlaceable.h
// Generated by format-carbon carbon-class --emit. Verify against interior/Tr2InteriorPlaceable.json.
import { carbon, impl, io, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";
import { vec3 } from "@carbonenginejs/core-math/vec3";
import { vec4 } from "@carbonenginejs/core-math/vec4";
import type { Vec3, Vec4 } from "@carbonenginejs/core-math/types";

/** Tr2InteriorPlaceable (interior) - generated from schema shapeHash 5fb7ec69.... */
@type.define({ className: "Tr2InteriorPlaceable", family: "interior" })
export class Tr2InteriorPlaceable extends CjsModel
{

  /** m_placeableResPath (std::string) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.string
  placeableResPath = "";

  /** m_transform (return) [READ, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.struct("return")
  transform = null;

  /** m_placeableRes (WodPlaceableResPtr) [READ] */
  @io.read
  @type.objectRef("WodPlaceableRes")
  placeableRes = null;

  /** m_display (bool) [READWRITE] */
  @io.readwrite
  @type.boolean
  display = true;

  /** m_boundingSphere[3] (Vector4) [READ] */
  @io.read
  @type.vec4
  boundingSphereRadius: Vec4 = vec4.create();

  /** m_depthOffset (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  depthOffset = 0;

  /** m_variableStore (Tr2VariableStorePtr) [READ] */
  @io.read
  @type.objectRef("Tr2VariableStore")
  variableStore = null;

  /** m_name (std::string) [READWRITE, PERSIST] */
  @io.persist
  @type.string
  name = "";

  /** m_probeOffset (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  probeOffset: Vec3 = vec3.create();

  /** m_isUniqueInstance (bool) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.boolean
  isUnique = false;

  /** Carbon method GetBoundingBoxInLocalSpace (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  GetBoundingBoxInLocalSpace(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorPlaceable", "GetBoundingBoxInLocalSpace", args);
  }

  /** Carbon method GetBoundingBoxInWorldSpace (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  GetBoundingBoxInWorldSpace(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorPlaceable", "GetBoundingBoxInWorldSpace", args);
  }

  /** Carbon method BoundingBoxOverride (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  BoundingBoxOverride(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorPlaceable", "BoundingBoxOverride", args);
  }

  /** Carbon method BoundingBoxReset (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  BoundingBoxReset(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("Tr2InteriorPlaceable", "BoundingBoxReset", args);
  }

}
