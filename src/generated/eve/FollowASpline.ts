// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Eve/SpaceObject/Children/Behaviors/FollowASpline.h
// Generated by format-carbon carbon-class --emit. Verify against eve/FollowASpline.json.
import { carbon, impl, io, schema, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";

/** FollowASpline (eve) - generated from schema shapeHash b9e40b05.... */
@type.define({ className: "FollowASpline", family: "eve" })
export class FollowASpline extends CjsModel
{

  /** m_priority (int32_t) [READWRITE, PERSIST, NOTIFY, ENUM] */
  @io.notify
  @io.persist
  @type.int32
  behaviorPriority = 0;

  /** m_tunnelGroupType (TunnelGroupType - enum TunnelGroupType) [READWRITE, PERSIST, ENUM] */
  @io.persist
  @type.int32
  @schema.enum("TunnelGroupType")
  tunnelGroupType = 0;

  /** m_splineTunnels (PSplineTunnelGroupVector) [READ, PERSIST] */
  @io.persist
  @type.list("SplineTunnelGroup")
  splineTunnels = [];

  /** m_smoothPullFactor (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  smoothPullFactor = 0.8;

  /** m_behaviorWeight (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  behaviorWeight = 600;

  /** m_enabled (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  enabled = true;

  /** m_cornerSmoothener (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  cornerSmoothener = 0.8;

  /** Carbon method remapTunnels -> UpdateTunnelRegistry (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  remapTunnels(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("FollowASpline", "remapTunnels", args);
  }

}
