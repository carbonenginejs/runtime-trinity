// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Eve/SpaceObject/Children/Behaviors/SpawnDrones.h
// Generated by format-carbon carbon-class --emit. Verify against eve/SpawnDrones.json.
import { carbon, impl, io, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";
import { vec3 } from "@carbonenginejs/core-math/vec3";
import { vec4 } from "@carbonenginejs/core-math/vec4";
import type { Vec3, Vec4 } from "@carbonenginejs/core-math/types";

/** SpawnDrones (eve) - generated from schema shapeHash 14b72cf1.... */
@type.define({ className: "SpawnDrones", family: "eve" })
export class SpawnDrones extends CjsModel
{

  /** m_gridSpacing (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  gridSpacing: Vec3 = vec3.create();

  /** m_gridFullnessFactor (float) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.float32
  gridFullnessFactor = 1;

  /** m_regenerateDrones (bool) [READWRITE] */
  @io.readwrite
  @type.boolean
  regenerateDrones = true;

  /** m_count (int) [READWRITE, PERSIST] */
  @io.persist
  @type.int32
  count = 1;

  /** m_seconds (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  seconds = -1;

  /** m_addOnGrid (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  addOnGrid = false;

  /** m_addByCount (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  addByCount = false;

  /** m_enabled (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  enabled = true;

  /** m_time (float) [READ, PERSIST] */
  @io.persist
  @type.float32
  time = 0;

  /** m_spawnPosition (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  spawnPosition: Vec3 = vec3.create();

  /** m_gridInfo (Vector4) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.vec4
  gridInfo: Vec4 = vec4.fromValues(1, 1, 1, 10);

  /** Carbon method gridToggleReset -> GridToggleReset (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  gridToggleReset(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("SpawnDrones", "gridToggleReset", args);
  }

}
