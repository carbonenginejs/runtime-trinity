// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Eve/SpaceObject/EveMissileWarhead.h
// Generated by format-carbon carbon-class --emit. Verify against eve/EveMissileWarhead.json.
import { carbon, impl, io, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";
import { vec3 } from "@carbonenginejs/core-math/vec3";
import type { Vec3 } from "@carbonenginejs/core-math/types";

/** EveMissileWarhead (eve) - generated from schema shapeHash 3502a67f.... */
@type.define({ className: "EveMissileWarhead", family: "eve" })
export class EveMissileWarhead extends CjsModel
{

  /** m_pathOffsetNoiseScale (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  pathOffsetNoiseScale = 0;

  /** m_pathOffsetNoiseSpeed (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  pathOffsetNoiseSpeed = 1;

  /** m_startDataValid (bool) [READWRITE] */
  @io.readwrite
  @type.boolean
  startDataValid = false;

  /** m_pathOffset (Vector3) [READWRITE] */
  @io.readwrite
  @type.vec3
  pathOffset: Vec3 = vec3.create();

  /** m_maxExplosionDistance (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  maxExplosionDistance = 40;

  /** m_impactDuration (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  impactDuration = 0.6;

  /** m_explosionPosition (Vector3) [READ] */
  @io.read
  @type.vec3
  explosionPosition: Vec3 = vec3.create();

  /** m_impactSize (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  impactSize = 0;

  /** m_spriteSet (EveSpriteSetPtr) [READWRITE, PERSIST] */
  @io.persist
  @type.objectRef("EveSpriteSet")
  spriteSet = null;

  /** m_targetLocator (int) [READ] */
  @io.read
  @type.int32
  targetLocatorID = -1;

  /** m_durationEjectPhase (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  durationEjectPhase = 0;

  /** m_doSpread (bool) [READWRITE] */
  @io.readwrite
  @type.boolean
  doSpread = true;

  /** m_acceleration (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  acceleration = 1;

  /** m_id (return) [READWRITE] */
  @io.readwrite
  @type.unknown
  id = -1;

  /** m_startEjectVelocity (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  startEjectVelocity = 0;

  /** m_warheadLength (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  warheadLength = 1;

  /** m_warheadRadius (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  warheadRadius = 1;

  /** Carbon method PrepareLaunch (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  PrepareLaunch(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveMissileWarhead", "PrepareLaunch", args);
  }

  /** Carbon method Launch (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  Launch(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveMissileWarhead", "Launch", args);
  }

}
