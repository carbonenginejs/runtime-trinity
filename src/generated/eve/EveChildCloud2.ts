// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Eve/SpaceObject/Children/EveChildCloud2.h
// Generated by format-carbon carbon-class --emit. Verify against eve/EveChildCloud2.json.
import { io, schema, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";
import { quat } from "@carbonenginejs/core-math/quat";
import { vec3 } from "@carbonenginejs/core-math/vec3";
import type { Quat, Vec3 } from "@carbonenginejs/core-math/types";

/** EveChildCloud2 (eve) - generated from schema shapeHash e973d7fa.... */
@type.define({ className: "EveChildCloud2", family: "eve" })
export class EveChildCloud2 extends CjsModel
{

  /** m_reflectionMode (EntityComponents::ReflectionMode - enum ReflectionMode) [READWRITE, PERSIST, NOTIFY, ENUM] */
  @io.notify
  @io.persist
  @type.int32
  @schema.enum("ReflectionMode")
  reflectionMode = 0;

  /** m_minVisibleQuality (Tr2VolumerticQuality - enum Tr2VolumerticQuality) [READWRITE, PERSIST, ENUM] */
  @io.persist
  @type.int32
  @schema.enum("Tr2VolumerticQuality")
  minVisibleQuality = 0;

  /** m_sortingModifier (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  sortingModifier = 0;

  /** m_animation (Tr2TextureAnimationPtr) [READWRITE, PERSIST] */
  @io.persist
  @type.objectRef("Tr2TextureAnimation")
  animation = null;

  /** m_shadowMapDS (Tr2DepthStencilPtr) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.objectRef("Tr2DepthStencil")
  shadowMapDS = null;

  /** m_lightMap (Tr2TextureReferencePtr) [READ] */
  @io.read
  @type.objectRef("Tr2TextureReference")
  lightmap = null;

  /** m_lightmapSizeScale (float) [READ] */
  @io.read
  @type.float32
  lightmapSizeScale = 0;

  /** m_lights (PTr2LightVector) [READ, PERSIST] */
  @io.persist
  @type.list("Tr2Light")
  lights = [];

  /** m_minScreenSize (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  minScreenSize = 0;

  /** m_rotation (Quaternion) [READWRITE, PERSIST] */
  @io.persist
  @type.quat
  rotation: Quat = quat.create();

  /** m_translation (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  translation: Vec3 = vec3.create();

  /** m_scaling (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  scaling: Vec3 = vec3.create();

  /** m_reflectionEffect (Tr2EffectPtr) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.objectRef("Tr2Effect")
  reflectionEffect = null;

  /** m_effect (Tr2EffectPtr) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.objectRef("Tr2Effect")
  effect = null;

  /** m_noiseTextureSize (uint32_t) [READWRITE, PERSIST] */
  @io.persist
  @type.uint32
  noiseTextureSize = 0;

  /** m_mapOffsets[0] (Vector3) [READ] */
  @io.read
  @type.vec3
  mapOffset0: Vec3 = vec3.create();

  /** m_mapOffsets[1] (Vector3) [READ] */
  @io.read
  @type.vec3
  mapOffset1: Vec3 = vec3.create();

  /** m_mapOffsets[2] (Vector3) [READ] */
  @io.read
  @type.vec3
  mapOffset2: Vec3 = vec3.create();

  /** m_castShadows (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  castShadows = false;

  /** m_receiveShadows (bool) [READWRITE, PERSIST] */
  @io.persist
  @type.boolean
  receiveShadows = false;

  /** m_name (std::string) [READWRITE, PERSIST] */
  @io.persist
  @type.string
  name = "";

  /** m_mapTiling[1] (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  detailTiling1: Vec3 = vec3.create();

  /** m_mapTiling[2] (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  detailTiling2: Vec3 = vec3.create();

  /** m_mapTiling[0] (Vector3) [READWRITE, PERSIST] */
  @io.persist
  @type.vec3
  textureTiling: Vec3 = vec3.create();

  /** m_display (bool) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.boolean
  display = false;

}
