// Generated by format-carbon carbon-class --emit-enums from schema enums.json.

export const AmbientOcclusionQuality = Object.freeze({
  Disabled: 0,
  Low: 1,
  Medium: 2,
  High: 3,
});

export const AntiAliasingQuality = Object.freeze({
  Disabled: 0,
  Low: 1,
  Medium: 2,
  High: 3,
});

export const BackgroundRenderingReason = Object.freeze({
  BACKGROUND_RENDER_COLOR: 0,
  BACKGROUND_RENDER_REFLECTION: 1,
});

export const BoundingSphereQuery = Object.freeze({
  EVE_BOUNDS_NORMAL: 0,
  EVE_BOUNDS_WITH_CHILDREN: 1,
});

export const EveVisualizeMethod = Object.freeze({
  VM_NONE: 0,
  VM_TEXCOORD0: 1,
  VM_TEXCOORD1: 2,
  VM_WHITE: 3,
  VM_OVERDRAW: 4,
  VW_WIREFRAME: 5,
  VW_LIGHT_COUNT: 6,
  VM_COUNT: 7,
});

export const ParameterType = Object.freeze({
  TYPE_EVESPACEOBJECT: 0,
  TYPE_EVEEFFECTROOT: 1,
  TYPE_ANYTHING: 2,
  TYPE_UNDEFINED: 3,
});

export const ReflectionMode = Object.freeze({
  REFLECT_HIGH: 0,
  REFLECT_MEDIUM_AND_HIGH: 1,
  REFLECT_LOW_MEDIUM_HIGH: 2,
  REFLECT_NEVER: 3,
});

export const ReflectionQuality = Object.freeze({
  Disabled: 0,
  Low: 1,
  Medium: 2,
  High: 3,
  Ultra: 4,
});

export const ReflectionSetting = Object.freeze({
  REFLECTION_SETTING_OFF: 0,
  REFLECTION_SETTING_LOW: 1,
  REFLECTION_SETTING_MEDIUM: 2,
  REFLECTION_SETTING_HIGH: 3,
  REFLECTION_SETTING_ULTRA: 4,
});

export const VisualizerType = Object.freeze({
  PIXEL_SHADER_REPLACEMENT: 0,
  FULL_SCREEN_QUAD: 1,
  FULL_SCREEN_QUAD_OVERLAY: 2,
});
