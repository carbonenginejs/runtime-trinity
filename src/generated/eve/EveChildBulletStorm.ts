// Ported from CarbonEngine (MIT, (c) 2026 CCP Games) - https://github.com/carbonengine/trinity
//   trinity/trinity/Eve/SpaceObject/Children/EveChildBulletStorm.h
// Generated by format-carbon carbon-class --emit. Verify against eve/EveChildBulletStorm.json.
import { carbon, impl, io, type } from "@carbonenginejs/core-types/schema";
import { CjsModel } from "@carbonenginejs/core-types/model";

/** EveChildBulletStorm (eve) - generated from schema shapeHash 31dfe6d2.... */
@type.define({ className: "EveChildBulletStorm", family: "eve" })
export class EveChildBulletStorm extends CjsModel
{

  /** m_targetObjects (PIEveSpaceObject2Vector) [READ, NOTIFY] */
  @io.notify
  @io.read
  @type.list("IEveSpaceObject2")
  targetObjects = [];

  /** m_objectCount (unsigned int) [READ] */
  @io.read
  @type.unknown
  objectCount = 0;

  /** m_clipSphere (float) [READ] */
  @io.read
  @type.float32
  clipSphere = 1;

  /** m_sourceRadius (float) [READ] */
  @io.read
  @type.float32
  sourceRadius = 0;

  /** m_sourceObject (EveSpaceObject2Ptr) [READWRITE, NOTIFY] */
  @io.notify
  @io.readwrite
  @type.objectRef("EveSpaceObject2")
  sourceObject = null;

  /** m_multiplier (uint32_t) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.uint32
  multiplier = 1;

  /** m_speed (float) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.float32
  speed = 1000;

  /** m_sourceLocatorSet (std::string) [READWRITE, PERSIST, NOTIFY] */
  @io.notify
  @io.persist
  @type.string
  sourceLocatorSet = "";

  /** m_name (std::string) [READWRITE, PERSIST] */
  @io.persist
  @type.string
  name = "";

  /** m_range (float) [READWRITE, PERSIST] */
  @io.persist
  @type.float32
  range = 1000;

  /** m_display (bool) [READWRITE] */
  @io.readwrite
  @type.boolean
  display = true;

  /** m_effect (Tr2EffectPtr) [READWRITE, PERSIST] */
  @io.persist
  @type.objectRef("Tr2Effect")
  effect = null;

  /** Carbon method CanChangeState (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  CanChangeState(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveChildBulletStorm", "CanChangeState", args);
  }

  /** Carbon method Rebuild (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  Rebuild(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveChildBulletStorm", "Rebuild", args);
  }

  /** Carbon method StartEffect (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  StartEffect(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveChildBulletStorm", "StartEffect", args);
  }

  /** Carbon method StopEffect (MAP_METHOD_AND_WRAP). */
  @carbon.method
  @impl.notImplemented
  StopEffect(...args: unknown[]): never
  {
    throw CjsModel.notImplemented("EveChildBulletStorm", "StopEffect", args);
  }

}
